...
<tr><td>Memory efficient as it can sort the array in place without needing to take up lots of memory. Equally suitable for EEPROM ram for example, where the number of writes is limited, since swapping is used intead of insertion with the Lomuto and Hoare methods.</td><td>While the number of assignment steps might be reduced, the number of iterations for an already sorted list will not be. This means that Quick Select works worst when compared to other selection methods when the list is already sorted.</td></tr>
